package com.yucei.admin.common.shiro;
...
public class KickoutSessionFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request,
        ...
        if (!subject.isAuthenticated() && !subject.isRemembered()) {
            logger.error("====当前用户认证状态:" + subject.isAuthenticated() + ",remebered:" + subject.isRemembered());
            // 如果没有登录，直接进行之后的流程
            subject.getSession().setTimeout(-1000l);
            //判断是不是Ajax请求，异步请求，直接响应返回未登录
            Result responseResult = Result.getInstance();
            if (ShiroFilterUtils.isAjax(request)) {
                if (logger.isDebugEnabled()) {
                    logger.debug(getClass().getName() + "当前用户已经在其他地方登录，并且是Ajax请求！");
                }
                responseResult.setStatus(String.valueOf(ExceptionEnum.ALREDY_LOGIN.getType()));
        ...
        try {
            // 当前用户
            Object sub = subject.getPrincipal();
            User user = (User) sub;
            ...
            if (logger.isInfoEnabled()) {
                logger.info("===放入登录session以后deque：==" + deque);
            }
    ...
    public static void out(ServletResponse response, Result result) {
        ...
        try {
            ...
            logger.error("用户在线数量限制【yucei-adminr-->KickoutSessionFilter.out】响应json信息成功");
        } catch (Exception e) {
            logger.error("用户在线数量限制【yucei-adminr-->KickoutSessionFilter.out】响应json信息出错", e);
        } finally {
            if (null != out) {
                out.flush();
                out.close();
    ...
    private boolean isAjaxResponse(ServletRequest request,
        ...
        if (ShiroFilterUtils.isAjax(request)) {
            ...
            responseResult.setStatus(ExceptionEnum.BAD_REQUEST.getType().toString());
            responseResult.setMessage("您已在别处登录，请您修改密码或重新登录");
            out(response, responseResult);
        } else {
            // 重定向
